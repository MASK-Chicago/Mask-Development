...
class FusionSC_Checklist {
	...
	public function render_parent( $args, $content = '' ) {
		...
		$defaults = FusionBuilder::set_shortcode_defaults(
			array(
				...
				'class'          => '',
				'id'             => '',
				'circle'         => strtolower( FusionBuilder::get_theme_option( 'checklist_circle' ) ),
				'circlecolor'    => FusionBuilder::get_theme_option( 'checklist_circle_color' ),
				'icon'           => 'fa-check',
				'iconcolor'      => FusionBuilder::get_theme_option( 'checklist_icons_color' ),
				'size'           => '13px',
			), $args
		);
	...
	public function attr() {
		...
		$attr['style'] = 'font-size:' . self::$parent_args['size'] . ';line-height:' . $line_height . 'px;';
	...
	public function item_content_attr() {
		return array(
			...
			'style' => 'margin-' . self::$parent_args['content_margin_position'] . ':' . self::$parent_args['content_margin'] . 'px;',
		);
	}
	...
	public function span_attr() {
		...
		if ( 'yes' === self::$child_args['circle'] || 'yes' === self::$parent_args['circle'] && ( 'no' !== self::$child_args['circle'] ) ) {
			...
			$attr['style'] .= 'font-size:' . self::$parent_args['circle_yes_font_size'] . 'px;';
		}

		$attr['class'] = 'icon-wrapper ' . $this->circle_class;

		$attr['style'] .= 'height:' . self::$parent_args['line_height'] . 'px;';
		$attr['style'] .= 'width:' . self::$parent_args['line_height'] . 'px;';
		$attr['style'] .= 'margin-' . self::$parent_args['icon_margin_position'] . ':' . self::$parent_args['icon_margin'] . 'px;';
	...
	public function icon_attr() {
		...
		if ( ! self::$child_args['icon'] ) {
			$icon = FusionBuilder::font_awesome_name_handler( self::$parent_args['icon'] );
		} else {
			$icon = FusionBuilder::font_awesome_name_handler( self::$child_args['icon'] );
		}
...
function fusion_element_checklist() {
	fusion_builder_map( array(
		...
		'preview'       => FUSION_BUILDER_PLUGIN_DIR . 'js/previews/fusion-checklist-preview.php',
